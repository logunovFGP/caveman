...
// native lanes resolve their roots from os.homedir(), not from --config-dir.
    // PATH matters as much as the dirs: isolatedEnv() sandboxes it, but callers
    // that pass no extraEnv inherited the real one, so `claude plugin uninstall`
    // ran against the developer's own account. It removed a real plugin twice.
    env: { ...process.env, CLAUDE_CONFIG_DIR: configDir, CLINE_DIR: path.join(configDir, 'cline-sandbox'), CLINE_DOCUMENTS_DIR: path.join(configDir, 'cline-documents-sandbox'), HERMES_HOME: path.join(configDir, 'hermes-sandbox'), PATH: hostlessPath(), NO_COLOR: '1', ...extraEnv },
    encoding: 'utf8',
  });
}
