...
| Cline | Native copy into `~/.cline` (or `$CLINE_DIR`): 6 skill dirs under `skills/`, the always-on ruleset at `rules/caveman.md`, and the 3 cavecrew presets as `agents/cavecrew-*.yaml`. Written through the `bin/lib/owned-install.js` journal, same as Hermes/opencode. Per-repo `.clinerules/caveman.md` via `--with-init` still works but is redundant once the global rule is installed. | Yes — `rules/` is injected into the system prompt every turn |
