...
| Cline | Native copy into **two roots**, because Cline reads them from two places and only skills honour `$CLINE_DIR`: every `skills/*/SKILL.md` except `caveman-stats` goes to `~/.cline/skills/` (auto-discovered, same wholesale rule as the Claude Code plugin — do NOT hand-maintain a list), while the always-on ruleset goes to `~/Documents/Cline/Rules/caveman.md` and the 3 cavecrew presets to `~/Documents/Cline/Agents/cavecrew-*.yaml`. Pinned from cline source (`disk.ts` `clineSkillsDir` / `ensureRulesDirectoryExists`, `AgentConfigLoader.getAgentsConfigPath`) — neither built bundle contains the string `.cline/rules` or `.cline/agents`, so the pre-fix layout was silently inert. Two ownership journals (`cline`, `cline-documents`); re-running migrates a legacy `$CLINE_DIR` rule/agents layout automatically. The documents root is overridable with `CLINE_DOCUMENTS_DIR` — cline resolves it through the VS Code documents path, which is not always `~/Documents`, and a root derived from `os.homedir()` alone is unsandboxable: a suite running a live `--uninstall` reached the developer's own `~/Documents/Cline` and deleted it. `tests/verify_repo.py::verify_cline_sandbox_pins` fails the build if any installer suite pins `CLINE_DIR` without it. `--with-mcp-shrink` also registers the proxy via `cline mcp add --yes`. Per-repo `.clinerules/caveman.md` via `--with-init` still works but is redundant once the global rule is installed. | Yes — `~/Documents/Cline/Rules/` is injected into the system prompt every turn |
