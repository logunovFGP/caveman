...
| Cline | Native copy into `~/.cline` (or `$CLINE_DIR`): every `skills/*/SKILL.md` except `caveman-stats` (auto-discovered, same wholesale rule as the Claude Code plugin — do NOT hand-maintain a list), the always-on ruleset at `rules/caveman.md`, and the 3 cavecrew presets as `agents/cavecrew-*.yaml`. `--with-mcp-shrink` also registers the proxy via `cline mcp add --yes`. Written through the `bin/lib/owned-install.js` journal, same as Hermes/opencode. Per-repo `.clinerules/caveman.md` via `--with-init` still works but is redundant once the global rule is installed. | Yes — `rules/` is injected into the system prompt every turn |
